...
- Cache pre-population:
  ...
  + `cache_httpfs_cache_parquet_file('s3://...', ['col1', 'col2'])` - Pre-populates the cache for only a subset of a parquet file's columns, emulating the cache footprint of `SELECT col1, col2 FROM read_parquet('s3://...')`. The column list accepts an `include :=` / `exclude :=` named argument (defaulting to `include`): `cache_httpfs_cache_parquet_file('s3://...', exclude := ['col1'])` warms every column except `col1`, emulating `SELECT * EXCLUDE (col1) FROM read_parquet('s3://...')`. Column names match top-level columns case-insensitively; unknown names are ignored. Column byte ranges are resolved through DuckDB's `parquet_metadata`, so the `parquet` extension must be loadable. Returns `TRUE` on success, `FALSE` if the file doesn't exist or its filesystem isn't wrapped by the cache, and throws if the file exists but can't be read as parquet.

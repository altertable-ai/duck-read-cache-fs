#include "include/nvmefs_temporary_block_manager.hpp"
#include <optional>
...
// Debug print functions - comment out if not used
#if 0
...
TemporaryBlock *NvmeTemporaryBlockManager::SplitBlock(TemporaryBlock *block, idx_t lba_amount) {
	...
	if (new_block->GetStartLBA() != allocated_start_lba) {
		...
		auto old_block = std::move(prev->next_block);

		new_block->previous_block = prev;             // Set the previous block to the new split block
		new_block->next_block = std::move(old_block); // Get current blocks unique_ptr and move it
		prev->next_block = std::move(new_block);      // Move the new block to the previous block
	} else {
		new_block->next_block = std::move(blocks); // Move the new block to the head of the list
		blocks = std::move(new_block);             // Move the new block to the head of the list
	}
...
void NvmeTemporaryBlockManager::CoalesceFreeBlocks(TemporaryBlock *block) {
	...
	if ((block->previous_block != nullptr && block->previous_block->IsFree()) &&
		...
		if (block->previous_block->previous_block != nullptr) {
			...
			unique_ptr<TemporaryBlock> old_left_block = std::move(block->previous_block->previous_block->next_block);

			block->previous_block->previous_block->next_block =
			    std::move(block->previous_block->next_block); // Move the previous block to the new merged block
			...
			blocks = std::move(block->previous_block->next_block);
			blocks->previous_block = nullptr; // Set the previous block to null
		}

		// Merge the next block
		unique_ptr<TemporaryBlock> old_right_block = std::move(block->next_block);
		...
		if (old_right_block->next_block != nullptr) {
			...
			block->next_block = std::move(old_right_block->next_block);
		}
